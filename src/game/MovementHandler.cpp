...
bool WorldSession::Anti__ReportCheat(const char* Reason,float Speed,const char* Op,float Val1,uint32 Val2)
{
    if(!Reason)
    {
        sLog.outError("Anti__ReportCheat: Absense de raison dans les parametres !");
        return false;
    }
    ...
    if(!Player)
    {
        sLog.outError("Anti__ReportCheat: Joueur sans pseudonyme ?");
        return false;
    }
...
bool WorldSession::Anti__CheatOccurred(uint32 CurTime,const char* Reason,float Speed,const char* Op, float Val1,uint32 Val2)
{
    if(!Reason)
    {
        sLog.outError("Anti__CheatOccurred: Absense de raison dans les parametres !");
        return false;
    }
