...
pAuraHandler AuraHandler[TOTAL_AURAS]=
{
    ...
    &Aura::HandleAuraModDisarm,                             //254 SPELL_AURA_MOD_DISARM_SHIELD disarm Shield
        ...
        switch (spellProto->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_DEATHKNIGHT:
            {
                ...
                if (spellProto->SpellFamilyFlags & UI64LIT(0x400080000000000))
                {
                    ...
                    else if (caster->HasSpell(49032))
                        m_modifier.m_amount += int32((caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.055f * 1.15f)*1.10f);  // Crypt Fever (Rank 1) - 10% more damage.
                    else
...
bool Aura::IsCritFromAbilityAura(Unit* caster, uint32& damage)
{
    ...
    {
        if(caster->IsSpellCrit(target, GetSpellProto(), GetSpellSchoolMask(GetSpellProto())))
        {
            damage = caster->SpellCriticalDamageBonus(GetSpellProto(), damage, target);
            return true;
        }
    }
